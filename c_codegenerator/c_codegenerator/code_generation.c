...
//  c_codegenerator
//
//  Created by parkhojeong on 2019/12/15.
...
char *opcode_name[]={
    ...
    "STO","STOB","STX","STXB",
    "SUBI","SUBF","DIVI","DIVF","ADDI","ADDF", "OFFSET","MULI",
...
{
   fprintf(fout,".global %5d ",addr);
   fprintf(fout," %d", node->value);
/*   gen_code_i(LDA,0,addr);
   gen_code_i(LITI,0,node->value);
   gen_code_i(STX,0,0);
   gen_code_i(POP,0,0);
*/
}
...
{
    gen_err++;
    printf("*** code generation error at line %d: ",ll);
    switch (i) {
